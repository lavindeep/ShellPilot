...
- **`/status` locality line.** After Model/Profile, `/status` shows `Locality: REMOTE — <host>` in amber when egressing (the non-loopback host, or `cloud` for a `-cloud` model on a loopback endpoint) or `Locality: local` in dim otherwise.
